...
class Graph:
    ...
    def remove_node(self, id: str, name: str) -> None:
        name = name.replace(',', '')
        if (id, name) in self.nodes:
            self.nodes.remove((id, name))
    ...
    def max_degree_nodes(self) -> dict:
        ...
        degree_dict = {k: v for k, v in sorted(degree_dict.items(), key=lambda x: x[1], reverse=True)}
        degree_dict = {k: v for k, v in degree_dict.items() if v == max(degree_dict.values())}
...
class  TMDBAPIUtils:
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        credit = connection.getresponse().read()
        ...
        # fix case where vote_average is not in credits
        for cred in credit:
            try:
                avg = cred['vote_average']
            except:
                cred['vote_average'] = 0.0

        if vote_avg_threshold is not None:
            credit = [cred for cred in credit if cred['vote_average'] >= vote_avg_threshold]
...
if __name__ == "__main__":
    ...
    graph = Graph()
    tmdb_api_utils = TMDBAPIUtils(api_key='32faaa84d58a522c6adce6acafa44a0e')
    # step 1
    graph.add_node(id='2975', name='Laurence Fishburne')
    Laurence_Fishburne_high = tmdb_api_utils.get_movie_credits_for_person(person_id='2975', vote_avg_threshold=8.0)
    for cred in Laurence_Fishburne_high:
        movie_id = str(cred['id'])
        cast_members = tmdb_api_utils.get_movie_cast(movie_id=movie_id, limit=3)
        for member in cast_members:
            graph.add_node(id=str(member['id']), name=member['name'])
    ...
    for i in range(2):
        if i == 0:
            print('iteration 1')
            graph.remove_node(id='2975', name='Laurence Fishburne')
            nodes = graph.nodes.copy()
            '''nodes = [('6384', 'Keanu Reeves'), ('530', 'Carrie-Anne Moss'), ('1107983', 'Martin Luther King Jr.'),
             ('52057', 'Obba Babatundé'), ('110380', 'Colin Powell'), ('3084', 'Marlon Brando'),
             ('3087', 'Robert Duvall'), ('8349', 'Martin Sheen'), ('16896', 'James Garner'), ('19239', 'Judith Ivey'),
             ('15532', 'Ruby Dee'), ('18471', 'Anthony Anderson'), ('74611', 'Tracee Ellis Ross'),
        ...
        for n, node in enumerate(nodes):
            # print(n, len(nodes))
            if n == len(nodes): break
            # print(f"Node: {n} -- {node}")
            credits = tmdb_api_utils.get_movie_credits_for_person(person_id=node[0], vote_avg_threshold=8.0)
            # print(len(credits))

            for credit in credits:
                top_3 = tmdb_api_utils.get_movie_cast(movie_id=credit['id'], limit=3)
                # print(len(top_3))

                for person in top_3:
                    graph.add_node(id=str(person['id']), name=person['name'])
                    nodes_added.append((str(person['id']), person['name']))
                    if node[0] != str(person['id']):
                        graph.add_edge(node[0], str(person['id']))
                # print("Done adding coactors")

    # print(graph.nodes)
    # print(tmdb_api_utils.get_movie_cast(movie_id='550', limit=5, exclude_ids=[7470]))
    # print(tmdb_api_utils.get_movie_cast(movie_id='603', limit=5))
    # print(tmdb_api_utils.get_movie_credits_for_person(person_id='287', vote_avg_threshold=7.3))
